...
cspl_svp=MIS(zvals510,coeffs_svp,k=5)
def def2_svp(z):
...
coeffs_tzvp=[np.array([g for ao in \
      gto.basis.load(bse.get_basis("def2-TZVP",fmt="nwchem",elements=[int(z)]),_symbol(int(z))) \
          for g in ao[1:]]) for z in zvals510]
cspl_tzvp=MIS(zvals510,coeffs_tzvp,k=5)
def def2_tzvp(z):
...
cspl_pvdz=[MIS(zvals310,c,k=5) for c in coeffs_pvdz_L]
def ccpvDZ(z):
...
cspl_pvtz=[MIS(zvals510,c,k=5) for c in coeffs_pvtz_L]
def ccpvTZ(z):
